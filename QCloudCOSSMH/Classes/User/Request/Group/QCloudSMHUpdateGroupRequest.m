...
@implementation QCloudSMHUpdateGroupRequest
...
- (BOOL)buildRequestData:(NSError *__autoreleasing *)error {
    ...
    if (!self.groupName ) {
        if (error != NULL) {
            *error = [NSError
                qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid
                             message:[NSString stringWithFormat:
                                                   @"InvalidArgument:paramter[groupName] is invalid (nil), it must have some value. please check it"]];
        }
        return NO;
    }
    ...
    self.requestData.directBody = [@{@"name":self.groupName} qcloud_modelToJSONData];
